 ...
 * www.welfordmedia.co.uk / help.welfordmedia.co.uk
 */
class ModelOpenstockOpenstock extends Model
{
    ...
    public function uninstall(){
        ...
        if($res->num_rows > 0){
			$query = $this->db->query("DROP TABLE `".DB_PREFIX."product_option_relation_discount_price`");
			$this->log->write('OPENSTOCK --> Dropped '.DB_PREFIX.'product_option_relation_discount_price table');
        }
    ...
    public function repair(){
        ...
        if($res->num_rows == 0){
            ...
            $res = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."product_option_relation` LIKE 'weight'");
            if($res->num_rows == 0){
                $this->db->query("ALTER TABLE `".DB_PREFIX."product_option_relation` ADD COLUMN `weight` decimal(15,8) NOT NULL AFTER `image`");
                $this->log->write('OPENSTOCK --> Altered '.DB_PREFIX.'product_option_relation table, added weight column');
            }
        }
        ...
        if($res->num_rows == 0){
            ...
            //check product_option_relation_group_price table columns
            $res = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."product_option_relation_group_price` LIKE 'date_start'");
            if($res->num_rows == 0){
                $this->db->query("ALTER TABLE `".DB_PREFIX."product_option_relation_group_price` ADD `date_start` date NOT NULL DEFAULT '0000-00-00'");
                $this->log->write('OPENSTOCK --> Altered '.DB_PREFIX.'product_option_relation_group_price table, added date_start');
            }

            $res = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."product_option_relation_group_price` LIKE 'date_end'");
            if($res->num_rows == 0){
                $this->db->query("ALTER TABLE `".DB_PREFIX."product_option_relation_group_price` ADD `date_end` date NOT NULL DEFAULT '0000-00-00'");
                $this->log->write('OPENSTOCK --> Altered '.DB_PREFIX.'product_option_relation_group_price table, added date_end');
            }
        }
        ...
        if($res->num_rows == 0){
            ...
            //check product_option_relation_discount_price table columns
            $res = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."product_option_relation_discount_price` LIKE 'date_start'");
            if($res->num_rows == 0){
                $this->db->query("ALTER TABLE `".DB_PREFIX."product_option_relation_discount_price` ADD `date_start` date NOT NULL DEFAULT '0000-00-00'");
                $this->log->write('OPENSTOCK --> Altered '.DB_PREFIX.'product_option_relation_discount_price table, added date_start');
            }

            $res = $this->db->query("SHOW COLUMNS FROM `".DB_PREFIX."product_option_relation_discount_price` LIKE 'date_end'");
            if($res->num_rows == 0){
                $this->db->query("ALTER TABLE `".DB_PREFIX."product_option_relation_discount_price` ADD `date_end` date NOT NULL DEFAULT '0000-00-00'");
                $this->log->write('OPENSTOCK --> Altered '.DB_PREFIX.'product_option_relation_discount_price table, added date_end');
            }
        }
    ...
    public function getProductOptionStocks($pId) {
        ...
        foreach ($mix_qry->rows as $row) {
            ...
            foreach ($grp_mix_qry->rows as $row2) {
				$gpArr[$row2['id']] = array(
					'customer_group_id' => $row2['customer_group_id'],
					'price' 			=> $row2['price'],
